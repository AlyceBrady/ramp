...
GRANT SELECT, INSERT, UPDATE, DELETE, TRIGGER ON `njala_dev`.* TO
    'smartuser'@'localhost', 'smartuser'@'%';
GRANT EXECUTE ON PROCEDURE `njala_dev`.`cancelstudentreg` TO
    'rampdba'@'localhost', 'smartuser'@'localhost', 'smartuser'@'%';
GRANT EXECUTE ON FUNCTION `njala_dev`.`termcensusdate` TO
    'rampdba'@'localhost', 'smartuser'@'localhost', 'smartuser'@'%';
GRANT EXECUTE ON FUNCTION `njala_dev`.`modofferingenddate` TO
    'rampdba'@'localhost', 'smartuser'@'localhost', 'smartuser'@'%';

--
-- Users, Resources, Roles, and Authorizations:
--
-- See smartNjalaSetup.sql for the table schemas, initial sample domain
-- users, and full set of access control rules for the tables defined in
-- the smartXxxSetup.sql files in this directory.
--
